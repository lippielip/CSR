...
async function dailyCheck() {
	...
	pool.getConnection(async function (err, connection) {
		...
		await connection.query(`SELECT * FROM presentation_status WHERE Year = ${currentWeek[0]} AND Calendar_Week = ${currentWeek[1]}`, async function (err, result) {
			...
			if (result.length === 0) {
				//create an entry for the week if it doesn't exist yet
				...
				//if it does exist then fill the status variable
			...
			async function (err, result, fields) {
				...
				if (status !== -1) {
					if (result.length === 0) {
						...
						mail(-1);
						connection.query(`UPDATE presentation_status SET Status = -1 WHERE Year = ${currentWeek[0]} AND Calendar_Week = ${currentWeek[1]}`, async function (err, result) {
							if (err) console.log(err)
						})
						...
						if (result.length === 1) {
							...
							connection.query(`UPDATE presentation_status SET Status = 0, Presenter1 = ${result[0].User_ID}, Presenter2 = NULL, Moderator = ${moderator.User_ID}  WHERE Year = ${currentWeek[0]} AND Calendar_Week = ${currentWeek[1]}`, async function (err, result) {
								if (err) console.log(err)
							})
						}
						...
						if (result.length === 2) {
							...
							connection.query(`UPDATE presentation_status SET Status = 0, Presenter1 = ${result[0].User_ID}, Presenter2 = ${result[1].User_ID}, Moderator = ${moderator.User_ID} WHERE Year = ${currentWeek[0]} AND Calendar_Week = ${currentWeek[1]}`, async function (err, result) {
								if (err) console.log(err)
							})
						}
						//Tuesday
